...
def resize_image_ocr_cstringio(cstring_object, num_rows, num_cols):
    ...
    # return cStringIO.StringIO(base64.decode(base64.encode(open_cv_image)))
    # return cStringIO.StringIO(open_cv_image)
    # thumb_str = io.StringIO(unicode(thumb2.tobytes(), 'utf16', errors='ignore'),)
    # #F_OUT.write(unicode(thumb_str))
    # return F_OUT

def resize_image_cstringio(cstring_object, num_rows, num_cols):
    F_IN = cstring_object
    # F_OUT = io.StringIO()
    size = (num_rows,num_cols)
    ...
    open_cv_image = open_cv_image[:,:].copy()
    return open_cv_image
    # return str(cStringIO.StringIO(str(base64.b64decode(base64.b64encode(np.array_str(open_cv_image))))))
    # return cStringIO.StringIO(open_cv_image)
    # thumb_str = io.StringIO(unicode(thumb2.tobytes(), 'utf16', errors='ignore'),)
    # #F_OUT.write(unicode(thumb_str))
    # return F_OUT
