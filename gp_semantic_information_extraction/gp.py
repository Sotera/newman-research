...
def exampletree2():
    return node( addw, [node(addw, [paramnode(0), constnode(5)]),
                        node(subw, [paramnode(0), constnode(2)])]
                )
...
def evolve(pc, popsize, rankfunction, maxgen=500,
    ...
    for i in range(maxgen):
        ...
        #   The two best always live on to next generation
        newpop = [scores[0][1], scores[1][1]]

        #   Build the next generation
        while len(newpop) < popsize:
            if random() > pnew:
                newpop.append(mutate(crossover(scores[selectindex()][1], scores[selectindex()][1],
                                               probswap=breedingrate), pc, probchange=mutationrate))
            else:
